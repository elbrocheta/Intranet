...
namespace Front.Controllers
{
    public class PrevencionRiesgosController : Controller
    {
        ...
        {
            ViewData["Modulo"] = (int)E_Modulos.PrevencionRiesgos;

            List<Noticias> _list = new List<Noticias>();

            HttpClient _client = WebApiHelper.p_APESAD_HttpClient();

            HttpResponseMessage _response = _client.GetAsync(WebApiHelper.ENDPOINT_NOTICIAS_GETBYTYPE + (int)E_NoticiaTipo.PrevencionRiesgos).Result;
