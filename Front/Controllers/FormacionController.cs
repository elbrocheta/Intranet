...
namespace Front.Controllers
{
    public class FormacionController : Controller
    {
        ...
        {
            ViewData["Modulo"] = (int)E_Modulos.Formacion;

            List<Noticias> _list = new List<Noticias>();

            HttpClient _client = WebApiHelper.p_APESAD_HttpClient();

            HttpResponseMessage _response = _client.GetAsync(WebApiHelper.ENDPOINT_NOTICIAS_GETBYTYPE + (int)E_NoticiaTipo.Formacion).Result;
